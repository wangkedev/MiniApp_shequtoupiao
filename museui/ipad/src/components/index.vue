...
<template>
  <div class="containerbox" id="container">
    <mu-row align-items='center' class='top'>
	    ...
	    <mu-col span="8">
	    	<mu-row align-items='center'>
	    		<div class='topfont'>目标用户任务量：<span>{{villagearr.length!=0?villagearr[villagearrindex].user_num:''}}</span></div>
	    		<div class='topfont'>已签用户量：<span>{{villagearr.length!=0?villagearr[villagearrindex].is_user_num:''}}</span></div>
	    	</mu-row>
	    </mu-col>
	  ...
	  <div class="mintop">
	  	<!--<mu-slide-top-transition>-->
			<midtop id='midtop' class='midtopflex' @gotop='gotop' @getvillagearr='getvillagearr' @getdata='getdata' @upinputdata='upinputdata' :searchval.sync='searchval' :screenarr='screenarr' :min_input.sync='min_input' :max_input.sync='max_input'></midtop>
	  	<!--</mu-slide-top-transition>-->
	  </div>
	  ...
	  <div class="box">
	  	<div v-for='(item,index) in list' :key='index' class="userbox" v-if='index>=(page-1)*pagesize&&index<page*pagesize'>
	  		...
	  		<div class="body">
	  			...
	  			<div class="bottom">
	  				<div class="left">
	  					<span v-if="item.is_rent == 2">已出租</span>
	  				</div>
	  ...
	  <div class="pagination" v-if="total>pagesize">
	   	<mu-pagination raised circle :total="total" :current.sync="page" :page-size="pagesize" @change='gotop'></mu-pagination>
		</div>
    ...
    <div class="selectvillagemask" v-show="selectvillagemask" @click='maskhide'>
    	<mu-col class='selectvillagebox' v-if='villagearr.length!=0'> 
    		<mu-col class='list' v-for='(item,index) in villagearr' :key='index' @click='villagearrindex = index;maskhide();'>{{item.e_name}}</mu-col>
    	</mu-col>
    </div>
   </mu-slide-top-transition>
    ...
    <mu-slide-bottom-transition>
   	...
   	<mu-slide-top-transition>
    	...
    	<div class="agementbox" v-if="agementboxshow">
    		...
    		<div class="btn" v-if='agementdata.is_rent == 1' @click='editstate("is_rent",2)'>设为已出租</div>
    		<div class="btn" v-if='agementdata.is_rent == 2' @click='editstate("is_rent",1)'>改为未出租</div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
      	...
      	banner:'http://pics.sc.chinaz.com/files/pic/pic9/201902/zzpic16641.jpg',
				...
				screenarr:[{ //筛选数组存放
					qianming:null,
					data:[{name:'不限签名',val:''},{name:'未签名',val:'0'},{name:'已签名',val:'1'},{name:'已回收',val:'2'},],
					index:0,
					show:false,
				},{ //筛选数组存放
					chuzu:null,
					data:[{name:'不限出租',val:''},{name:'未出租',val:'1'},{name:'已出租',val:'2'}],
					index:0,
					show:false,
				}],
     ...
     */
    	 ...
    	 */
    	goautograph(){
    		this.$router.push({name:'签名',params:{villagearrindex:this.villagearrindex,agementdata:this.agementdata}});
    		this.maskhide();
    	},
    	/**
    	 * 去修改信息
    	 */
    	goedit(){
    		this.$router.push({path:'/edit',query:{villagearrindex:this.villagearrindex,agementdata:this.agementdata}});
    		this.maskhide();
    	},
    	/**
    	 * 子组件将数据更新到这里
    	 ...
    	 */
    	getdata(){
    		let list = JSON.parse(localStorage.getItem('data'))[this.villagearrindex].list,arr=[];
    		list.forEach((v,i)=>{
    			if(
    			(this.screenarr[0].data[this.screenarr[0].index].val===''||v.vote_type ==this.screenarr[0].data[this.screenarr[0].index].val)&&
    			(this.screenarr[1].data[this.screenarr[1].index].val===''||v.is_rent ==this.screenarr[1].data[this.screenarr[1].index].val)&&
    			((this.min_input?Number(this.min_input):0)<=Number(v.serial)&&Number(v.serial)<=(this.max_input?Number(this.max_input):99999999999))&&
    			(v.address.indexOf(this.searchval) != -1||v.name.indexOf(this.searchval)!= -1)
    			){
    				arr.push(v);
    			}
    		})    
    	...
    	getvillagearr(){
    		...
    		JSON.parse(localStorage.getItem('data')).forEach((v,i)=>{
    			this.villagearr.push({e_name:v.e_name,user_num:v.user_num,is_user_num:v.is_user_num})
    		})
    	},
    	/**
    	 * 退出
