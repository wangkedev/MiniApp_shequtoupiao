<template>
   ...
   <mu-row class='mid'>
  	<div class='title'>
		<div class="left">用户签名<img src="../assets/img/icon_bluerectangle.svg" alt="" /><span @click="goallsj">已投数据</span></div>
		<div class="right">
...
//	          	this.end = true;
    	...
    	up(data){
			var listarr = JSON.parse(localStorage.getItem('alldata1'))
			this.$post('/phone/Drop/state_dada',{gogobao:listarr}).then(res=>{
				console.log('已投数据上传成功')
			}).catch(res=>{
				this.password = '';
				this.passworddialog = false;
	        	this.$toast.error('网络异常');
				...
				console.log('上传数据data：',data)
				console.log('上传数据u_id：',localStorage.getItem('id'))
